...
const percentatgeAssignades = computed(() => {
    ...
    return Math.round((inscripcionsAssignades.value / totalInscripcions.value) * 100);
});

const percentatgePendents = computed(() => {
    if (totalInscripcions.value === 0) return 0;
    return Math.round((inscripcionsPendents.value / totalInscripcions.value) * 100);
});
