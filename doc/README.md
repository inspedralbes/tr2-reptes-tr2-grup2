...
2.  **Gestió via Body:** La informació necessària per a les operacions es transmet principalment a través del _body_ de la petició HTTP.
...
El següent que hauràs de fer és escriure a la teva consola `> npx prisma migrate` i `dev` si es per a desenvolupament o `deploy` si és per a producció i afegir `--name [nom de la migració]`
...
En cas de fer servir un servei de control de versions, elimineu la carpeta que es genera de migracions per evitar duplicacions o modificacions. Opcionalment també es pot fer un .gitignore.
